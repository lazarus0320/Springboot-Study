...
public class JwtAuthenticationProcessingFilter extends OncePerRequestFilter {
    ...
    /**
     * 리프래시 토큰 재발급 & db에 리프레시 토큰 업데이트 메소드
     * jwtService.createRefreshToken()으로 리프레시 토큰을 재발급 후
     * db에 재발급한 리프레시 토큰 업데이트 및 Flush
     */
    ...
    /**
     * 액세스 토큰 체크 & 인증 처리 메소드
     * request에서 extractAccessToken()으로 액세스 토큰 추출 후, isTokenValid()로 유효 토큰 검증
     * 유효 토큰이면 액세스 토큰에서 extractEmail로 Email을 추출, findByEmail()로 해당 이메일 사용하는 유저 반환
     * 그 유저 객체를 saveAuthentication()으로 인증처리하고 인증 허가 처리된 객체를 SecurityContextHolder에 담기
     * 그 후 다음 인증 필터로 진행
     */
    ...
    public void saveAuthentication(User myUser) {
        ...
        if (password == null) { // 소셜 로그인 유저의 비밀번호를 임의로 설정해서 소셜 로그인 유저도 인증이 되도록 일단 설정
        ...
        UserDetails userDetailsUser = org.springframework.security.core.userdetails.User.builder()
                ...
                .password(password)
                .roles(myUser.getRole().name()) // 사용자의 역할을 가져와 문자열 형태로 설정
                .build();
